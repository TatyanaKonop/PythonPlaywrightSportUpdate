...
class Navbar:
    ...
    def navigate_to_section_on_main_menu(self, sections_text: list) -> None:
        ...
        ListItem(section_locator, name=f'section_{sections_text[0].lower()}').click()
        ...
        Button(submenu_locator, name=f'section_{sections_text[1].lower()}').click()


    def navigate_to_section_on_top_menu(self, sections_text: list) -> None:
        xpath_query_on_top = f"//a[text()=('{sections_text[0]}')]"
        with self.top_menu_root.page.expect_popup() as page1_info:
